...
class TalentPolicyExtractor:
    ...
    RESEARCH_FUND_PATTERNS = [
        r"科研启动(?:经费|金|资助)(?:最高)?(?:达)?([0-9]+(?:\.[0-9]+)?(?:万元|万|w))",
        r"科研经费(?:最高)?(?:达)?([0-9]+(?:\.[0-9]+)?(?:万元|万|w))",
        r"提供科研启动经费([0-9]+(?:-[0-9]+)?万元)"
    ]

    # 户口解决与配偶安置
    BENEFITS_KEYWORDS = {
        "解决编制": ["落实事业编制", "纳入实名制编制", "全额事业编制", "入编", "直聘事业编制"],
        "户口解决": ["解决北京户口", "解决上海户口", "落户", "解决户口", "协助落户", "配偶随迁"],
        "住房保障": ["提供人才公寓", "提供过渡住房", "免租金", "周转房", "人才周转房", "人才周转公寓", "周转公寓", "免租人才公寓", "人才公寓", "住房保障", "配租"],
        "子女入学": ["协助解决子女入学", "解决子女就读", "子女优质学位", "子女入学优待", "解决配偶工作及子女优质入学", "协调解决配偶工作", "子女入学"]
    }
