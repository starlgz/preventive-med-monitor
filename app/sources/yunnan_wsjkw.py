...
class YunnanWsjkwSource(BaseSource):
    """
    云南省卫生健康委员会 / 云南省疾控与公卫事业单位招考源插件
    覆盖云南省疾控中心、昆明市及各州市疾控、地方卫健机构预防医学岗位
    """
    source_id = "yunnan_wsjkw"
    name = "云南省卫生健康委员会-人事招聘"
    category = "official"
    province = "云南"
    base_url = "http://ynswsjkw.yn.gov.cn/wjwWebsite/web/doc/UU10/docIndex.html"
    interval_minutes = 60
        ...
        async with await self.get_http_client() as client:
            for page in range(1, max_pages + 1):
                ...
                try:
                    ...
                    items = soup.select(".list-ul li, .news-list li, .doc-list li, ul.list li")
                    if not items:
                    ...
                    for item in items:
                        ...
                        if not any(k in title for k in ["招聘", "招考", "选聘", "引进", "招录", "拟聘", "聘用", "考核"]):
                            continue
                        
                        if href.startswith("./"):
                            href = "http://ynswsjkw.yn.gov.cn/wjwWebsite/web/doc/UU10/" + href[2:]
                        elif href.startswith("/"):
                            href = "http://ynswsjkw.yn.gov.cn" + href
                        elif not href.startswith("http"):
                            href = "http://ynswsjkw.yn.gov.cn/wjwWebsite/web/doc/UU10/" + href
    ...
    async def fetch_detail(self, announcement_url: str) -> Optional[RawAnnouncementDetail]:
        try:
            async with await self.get_http_client() as client:
                ...
                title = title_node.get_text(strip=True) if title_node else "云南省卫健委招聘公告"
