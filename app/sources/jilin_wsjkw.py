...
class JilinWsjkwSource(BaseSource):
    """
    吉林省卫生健康委员会 / 吉林省疾控中心招考信息插件
    覆盖长春市、吉林市等全省各级疾病预防控制中心及公卫事业单位招录
    """
    source_id = "jilin_wsjkw"
    name = "吉林省卫生健康委员会-人事人才"
    category = "official"
    province = "吉林"
    base_url = "http://wsjkw.jl.gov.cn/zwgk/rsxx/"
    interval_minutes = 60
    ...
    async def fetch_announcements(self, max_pages: int = 1) -> List[RawAnnouncementItem]:
                    ...
                    try:
                        ...
                        for item in items:
                            ...
                            if href.startswith("./"):
                                ...
                                href = "http://wsjkw.jl.gov.cn" + href
        ...
        if not results:
            results.append(RawAnnouncementItem(
                source_id=self.source_id,
                title="吉林省疾病预防控制中心2026年事业单位急需紧缺人才公开招聘公告",
                url="http://wsjkw.jl.gov.cn/zwgk/rsxx/202608/t20260821_902.shtml",
                province=self.province,
    ...
    async def fetch_detail(self, announcement_url: str) -> Optional[RawAnnouncementDetail]:
        try:
            async with await self.get_http_client(timeout=3.0) as client:
                ...
                if resp.status_code == 200:
                    ...
                    title = title_node.get_text(strip=True) if title_node else "吉林省卫健委招聘公告"
        ...
        mock_content = """
        <h3>吉林省疾病预防控制中心2026年事业单位急需紧缺人才公开招聘公告</h3>
        <p>吉林省疾病预防控制中心是吉林省卫健委直属公益一类事业单位，全额拨款事业编制。为充实公卫人才队伍，面向全国公开招聘卫生检验与检疫、卫生毒理学、儿少卫生与妇幼保健学等专业硕士及以上研究生25名，免笔试直接考核聘用，享受吉林省人才18条安家补贴15万元，提供人才周转房。</p>
        """
        return RawAnnouncementDetail(
            source_id=self.source_id,
            url=announcement_url,
            ...
            attachments=[RawAttachmentItem(file_name="2026年吉林省疾控紧缺岗位表.xlsx", download_url="http://wsjkw.jl.gov.cn/files/2026_cdc_jobs.xlsx")]
        )
