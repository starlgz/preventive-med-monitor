...
class GuizhouWsjkwSource(BaseSource):
    """
    贵州省卫生健康委员会 / 贵州省疾控招考信息源插件
    包含贵州全省及各市州疾控中心、公立医疗卫生机构、公共卫生事业单位招考
    """
    source_id = "guizhou_wsjkw"
    name = "贵州省卫生健康委员会-人事招聘"
    category = "official"
    province = "贵州"
    base_url = "http://wjw.guizhou.gov.cn/xwzx/tzgg/"
    interval_minutes = 60
        ...
        async with await self.get_http_client() as client:
            for page in range(1, max_pages + 1):
                ...
                try:
                    ...
                    items = soup.select(".right_list li, .gl-list li, .list_con li, ul.news_list li")
                    if not items:
                    ...
                    for item in items:
                        ...
                        if href.startswith("./"):
                            ...
                            href = "http://wjw.guizhou.gov.cn" + href
    ...
    async def fetch_detail(self, announcement_url: str) -> Optional[RawAnnouncementDetail]:
        try:
            async with await self.get_http_client() as client:
                ...
                title = title_node.get_text(strip=True) if title_node else "贵州省卫健委招聘公告"
